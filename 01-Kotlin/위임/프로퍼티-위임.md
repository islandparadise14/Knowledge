---
tags: [kotlin, delegation, property]
---

# 프로퍼티 위임 (Property Delegation)

## 💡 핵심 개념

**프로퍼티 위임**은 프로퍼티의 getter/setter 로직을 별도 객체에 위임하는 패턴이다. `by` 키워드를 사용하며, 반복되는 프로퍼티 로직을 재사용 가능하게 만든다.

## 📌 왜 필요한가?

프로퍼티마다 비슷한 초기화, 검증, 변환 로직을 반복 작성하면 보일러플레이트가 늘어난다. 위임을 사용하면 이런 공통 패턴을 한 번만 정의하고 여러 프로퍼티에 재사용할 수 있다. Android에서는 `by lazy`, `by viewModels()`, `by remember` 등 거의 필수적으로 사용된다.

## 🔍 자세히

### 기본 사용법

```kotlin
// by 키워드로 프로퍼티 위임
class Example {
    // lazy - 처음 접근 시 초기화
    val heavyObject: HeavyObject by lazy {
        HeavyObject()
    }

    // observable - 값 변경 감지
    var name: String by Delegates.observable("초기값") { property, old, new ->
        println("$old -> $new")
    }

    // vetoable - 값 변경 거부 가능
    var age: Int by Delegates.vetoable(0) { _, _, new ->
        new >= 0  // false 반환 시 변경 거부
    }
}
```

### 동작 원리

**위임 프로퍼티는 내부적으로 `getValue`/`setValue` 호출로 변환된다:**

```kotlin
// 이 코드는:
class MyClass {
    var prop: String by MyDelegate()
}

// 컴파일러가 이렇게 변환:
class MyClass {
    private val prop$delegate = MyDelegate()

    var prop: String
        get() = prop$delegate.getValue(this, this::prop)
        set(value) = prop$delegate.setValue(this, this::prop, value)
}
```

**커스텀 위임 만들기:**

```kotlin
import kotlin.reflect.KProperty

// ReadWriteProperty 인터페이스 구현
class TrimDelegate : ReadWriteProperty<Any?, String> {
    private var trimmedValue: String = ""

    override fun getValue(thisRef: Any?, property: KProperty<*>): String {
        return trimmedValue
    }

    override fun setValue(thisRef: Any?, property: KProperty<*>, value: String) {
        trimmedValue = value.trim()
    }
}

// 사용
class UserForm {
    var name: String by TrimDelegate()
    var email: String by TrimDelegate()
}

val form = UserForm()
form.name = "  민트  "
println(form.name)  // "민트"
```

### 표준 라이브러리 위임

**1. lazy**

```kotlin
// 기본 - thread-safe (SYNCHRONIZED)
val config: Config by lazy {
    loadConfig()
}

// 단일 스레드 환경이면 NONE으로 성능 향상
val adapter: Adapter by lazy(LazyThreadSafetyMode.NONE) {
    Adapter()
}

// PUBLICATION - 여러 스레드가 동시에 초기화 가능하지만 첫 번째 결과만 사용
val cache: Cache by lazy(LazyThreadSafetyMode.PUBLICATION) {
    buildCache()
}
```

**2. observable**

```kotlin
var count: Int by Delegates.observable(0) { prop, old, new ->
    println("${prop.name}: $old -> $new")
    onCountChanged(new)
}

count = 5   // "count: 0 -> 5"
count = 10  // "count: 5 -> 10"
```

**3. vetoable**

```kotlin
var percentage: Int by Delegates.vetoable(0) { _, _, new ->
    new in 0..100  // 0~100 범위만 허용
}

percentage = 50   // OK
percentage = 150  // 거부됨, 여전히 50
```

**4. notNull**

```kotlin
// lateinit과 유사하지만 primitive 타입에 사용 가능
var count: Int by Delegates.notNull<Int>()

// 초기화 전 접근하면 IllegalStateException
```

### Map을 통한 위임

```kotlin
// Map에서 프로퍼티 값을 가져오기
class User(map: Map<String, Any?>) {
    val name: String by map
    val age: Int by map
}

val user = User(mapOf("name" to "민트", "age" to 27))
println(user.name)  // "민트"
println(user.age)   // 27

// MutableMap이면 var도 가능
class MutableUser(map: MutableMap<String, Any?>) {
    var name: String by map
    var age: Int by map
}
```

### provideDelegate

```kotlin
// 위임 객체 생성 시점에 검증 로직 추가
class ResourceLoader(val prefix: String) {
    operator fun provideDelegate(
        thisRef: Any?,
        prop: KProperty<*>
    ): ReadOnlyProperty<Any?, String> {
        // 프로퍼티 이름 기반으로 리소스 검증
        val resourceKey = "$prefix.${prop.name}"
        require(resourceExists(resourceKey)) {
            "Resource $resourceKey not found"
        }
        return ResourceProperty(resourceKey)
    }
}

class App {
    val title by ResourceLoader("strings")   // strings.title 검증
    val version by ResourceLoader("config")   // config.version 검증
}
```

### 주의사항

**1. lazy는 val만 가능**

```kotlin
// lazy는 val 전용
val value: String by lazy { "Hello" }  // OK
// var value: String by lazy { "Hello" }  // ERROR
```

**2. 위임 프로퍼티와 backing field**

```kotlin
// 위임 프로퍼티는 backing field를 가지지 않음
class MyClass {
    var prop: String by MyDelegate()
    // field 키워드 사용 불가
}
```

**3. 인터페이스 프로퍼티 위임**

```kotlin
interface HasName {
    val name: String
}

// 인터페이스 프로퍼티도 위임 가능
class NamedEntity(map: Map<String, Any?>) : HasName {
    override val name: String by map
}
```

## 💻 실전 예제

### SharedPreferences 위임

```kotlin
class PreferenceDelegate<T>(
    private val prefs: SharedPreferences,
    private val key: String,
    private val defaultValue: T
) : ReadWriteProperty<Any?, T> {

    @Suppress("UNCHECKED_CAST")
    override fun getValue(thisRef: Any?, property: KProperty<*>): T {
        return when (defaultValue) {
            is String -> prefs.getString(key, defaultValue) as T
            is Int -> prefs.getInt(key, defaultValue) as T
            is Boolean -> prefs.getBoolean(key, defaultValue) as T
            is Long -> prefs.getLong(key, defaultValue) as T
            is Float -> prefs.getFloat(key, defaultValue) as T
            else -> throw IllegalArgumentException("Unsupported type")
        }
    }

    override fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
        prefs.edit {
            when (value) {
                is String -> putString(key, value)
                is Int -> putInt(key, value)
                is Boolean -> putBoolean(key, value)
                is Long -> putLong(key, value)
                is Float -> putFloat(key, value)
            }
        }
    }
}

// 확장 함수로 편리하게
fun <T> SharedPreferences.delegate(key: String, defaultValue: T) =
    PreferenceDelegate(this, key, defaultValue)

// 사용
class SettingsManager(prefs: SharedPreferences) {
    var isDarkMode: Boolean by prefs.delegate("dark_mode", false)
    var userName: String by prefs.delegate("user_name", "")
    var fontSize: Int by prefs.delegate("font_size", 14)
}
```

### ViewModel 위임 (Android KTX)

```kotlin
class MainActivity : AppCompatActivity() {
    // Activity에서 ViewModel 위임
    private val viewModel: MainViewModel by viewModels()

    // Factory 사용
    private val detailViewModel: DetailViewModel by viewModels {
        DetailViewModel.Factory(userId)
    }
}

class UserFragment : Fragment() {
    // Fragment에서 ViewModel 위임
    private val viewModel: UserViewModel by viewModels()

    // Activity 범위 ViewModel 공유
    private val sharedViewModel: SharedViewModel by activityViewModels()
}
```

### Compose에서의 위임

```kotlin
@Composable
fun CounterScreen() {
    // remember + mutableStateOf를 by로 위임
    var count by remember { mutableStateOf(0) }
    var text by remember { mutableStateOf("") }

    Column {
        Text("Count: $count")
        Button(onClick = { count++ }) {
            Text("Increment")
        }
        TextField(
            value = text,
            onValueChange = { text = it }
        )
    }
}
```

### Bundle Argument 위임

```kotlin
// Fragment argument를 위임으로 깔끔하게
fun <T> Fragment.argument(key: String): ReadOnlyProperty<Fragment, T> =
    ReadOnlyProperty { thisRef, _ ->
        @Suppress("UNCHECKED_CAST")
        thisRef.requireArguments().get(key) as T
    }

fun <T> Fragment.argumentOrNull(key: String): ReadOnlyProperty<Fragment, T?> =
    ReadOnlyProperty { thisRef, _ ->
        @Suppress("UNCHECKED_CAST")
        thisRef.arguments?.get(key) as? T
    }

// 사용
class UserFragment : Fragment() {
    private val userId: String by argument("user_id")
    private val title: String? by argumentOrNull("title")

    companion object {
        fun newInstance(userId: String) = UserFragment().apply {
            arguments = bundleOf("user_id" to userId)
        }
    }
}
```

## 🔗 관련 개념

- [[01-Kotlin/기본문법/변수와-타입|변수와 타입 - lateinit vs lazy]]
- [[01-Kotlin/클래스-종류/Object와-Companion|Object와 Companion]]

## 📚 더 보기

- [Kotlin Docs - Delegated Properties](https://kotlinlang.org/docs/delegated-properties.html)
- [Kotlin Docs - Delegation](https://kotlinlang.org/docs/delegation.html)

---

**핵심 요약:** `by` 키워드로 프로퍼티 로직을 위임. lazy, observable, vetoable은 표준 제공. 커스텀 위임으로 SharedPreferences, Bundle 등 보일러플레이트 제거. Android에서는 viewModels(), remember 등 필수 패턴.
