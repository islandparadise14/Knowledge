...
**제네릭(Generics)**은 타입을 파라미터화해서 코드 재사용성을 높이고 타입 안전성을 보장한다. `List<T>`, `Flow<T>` 같은 형태로 사용한다.

## 📌 왜 필요한가?

같은 로직을 여러 타입에 적용할 수 있다. 컴파일 타임에 타입을 체크해서 런타임 에러를 방지한다. 캐스팅 없이 타입 안전하게 작업 가능하다.
...
fun <T> copy(from: T, to: T) where T : Cloneable, T : Comparable<T> {
    // T는 Cloneable이면서 Comparable이어야 함
}
...
// Dog의 Producer는 Animal의 Producer로 취급 가능
...
// Animal의 Consumer는 Dog의 Consumer로 취급 가능
...
// MutableList<*>는 MutableList<out Any?>와 유사
// 읽기만 가능, 쓰기 불가
```
...
**핵심 요약:** 제네릭으로 타입 안전성 보장. out(생산), in(소비). 타입 소거 주의. reified로 타입 체크.
